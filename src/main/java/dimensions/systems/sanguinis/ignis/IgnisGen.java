...
package dimensions.systems.sanguinis.ignis;
...
public class IgnisGen extends ChunkGenerator {
    int currentHeight;
    ...
    public ChunkData generateChunkData(final World world, final Random random, final int chunkX, final int chunkZ, final BiomeGrid biome) {
        ...
        for (int X = 0; X < 16; ++X) {
            for (int Z = 0; Z < 16; ++Z) {
                ...
                //Oberster Layer
                if(celutis.randomrange(0, 9) > 3){
                    chunk.setBlock(X, currentHeight, Z, Material.LAVA);
                }else if(celutis.randomrange(0, 9) > 2) {
