...
package dimensions.systems.sanguinis.rocky;
...
public class RockyGen extends ChunkGenerator {
    int currentHeight;
    ...
    public List<BlockPopulator> getDefaultPopulators(World world) {
        ...
        return List.of(new RockyPop(), new CommonSolarStructurePop(), new SimpleCavePopulator());
    ...
    public ChunkData generateChunkData(final World world, final Random random, final int chunkX, final int chunkZ, final BiomeGrid biome) {
        ...
        for (int X = 0; X < 16; ++X) {
            for (int Z = 0; Z < 16; ++Z) {
                ...
                currentHeight += (int) (Math.tanh(fnl.GetNoise(chunkX * 16 + X, chunkZ * 16 + Z) * Math.PI) * 40);
